...
function Vehicles() {
    ...
    if (fetchLoading) {
        ...
        vehiclesData = vehicles.map((vehicle, index) => (
            <>
                {(selectedVehicletype === "all" || vehicle.vehicle_type_name === selectedVehicletype) &&
                    (selectedStatus === "all" || vehicle.vehicle_status === selectedStatus) &&
                    (searchBar === "" || vehicle.model.toLowerCase().includes(searchBar.toLowerCase())) &&
                    (
                        <tr key={vehicle.id}>
                            <th>{vehicle.id}</th>
                            <td>{vehicle.model}</td>
    ...
    return (
        <>
            <div className="flex flex-col">
                ...
                <input className="input input-bordered join-item bg-base-200 w-2/4" onChange={(e) => { setSearchBar(e.target.value) }} placeholder="Search model" />
            </div>
